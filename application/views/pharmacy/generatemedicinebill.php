  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <center><h1 class="m-0 text-dark">Generate Medicine Bill</h1></center>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Generate Medicine Bill</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
...
<section class="content">
  <h3 class="text-center">Generate Medicine Bill</h3>
     <!--    <?php //echo form_open('doctorController/addDoctor');?> -->

		<form action="<?= base_url('pharmacyController/addsalemedicine'); ?>" enctype="multipart/form-data" method="post">
		           <div class="row">
				  <!-- <div class="col-md-4"  style="margin-button 15px;">
				     ...
				     <div class="col-md-4"  style="margin-button 15px;">
						  ...
						  <select value="" name="name" class="form-control chosen patientlist">		 <option>select</option>
						  <?php foreach($patient as $pt){ ?>
						  <option value="<?= $pt->uhid; ?>"><?= $pt->first_name. ' '.$pt->middle_name.' '.$pt->last_name; ?> </option>
						  <?php } ?>
						  </select>
						  <?php if (isset($this->session->flashdata('error1')['name'])) 
									echo $this->session->flashdata('error1')['name'];?>
                    </div>
					<div class="col-md-4 d-none cstmdoctorhos"  style="margin-button 15px;">
						  <label>Hospital Doctor</label>
						  <select value="" name="hdoctor" class="form-control  doctorlist">		 
						  <?php foreach($doctor as $dt){ ?>
						  <option value="<?= $dt->doc_name; ?>"><?= $dt->doc_name; ?> </option>
						  <?php } ?>
						  </select>
						  <?php if (isset($this->session->flashdata('error1')['name'])) 
									echo $this->session->flashdata('error1')['name'];?>
					 ...
					 <div class="col-md-4"  style="margin-button 15px;">
						  ...
						  <input type="text" value="" name="doctor" class="form-control doctordata">
					 ...
					 <div class="col-md-4"  style="margin-button 15px;">
						  <label>Date</label>
						  <input type="text" value="<?= $time; ?>" name="tdate" class="form-control">
					  ...
					  <div class="col-md-12">
                        <table class="table table-bordered" id="item_table" style="width:100%;     margin-top: 50px;" align="center">
							...
							<tr>
                                ...
                                <th style="font-size: 15px;">Medicine Price<span style="color:red">*</span></th>
								<th style="font-size: 15px; width: 11%;">Quantity<span style="color:red">*</span>Available Quantity</th>
				             ...
				             <tr class="clsmttr">
								...
								<td><input type="text" name="expdate[]" value="" id="expdate" class="form-control expdate "  required="" >
								</td>
								<td><input type="text" name="medprice[]" id="medprice" class="form-control medprice"  required="" value="" >
								</td>
								<td  style="display: flex;"><input type="text" name="medquan[]" id="medquan" value="" class="form-control medquan"  required="" style="width:50%;"><input type="text" name="medquanav[]" id="medquanav" class="form-control medquanav" value="" required=""style="width:50%;" >
								</td>
					...
					<div class="col-md-3">
					<label><b><br>Bill No</b></label>
					<input type="text" class="form-control" name="bill" value="<?= $bill; ?>">
				     </div>
...
$("select.patientlist").change(function(){
	 ...
	 $.ajax({
		...
		$("select.mednames").change(function(){
        var medname = $(this).children("option:selected").val();
	 $.ajax({
			url:"<?=base_url('pharmacyController/getproductdetails')?>",
			type:'POST',
			data: {medname:medname},
			success: function(response){
				$(".customproductdata").html(response);
			var batch_no = $('.cbatch_no').val();
			var avalablequan = $('.cavalablequan').val();
			var expdate = $('.cexpdate').val();
			var price = $('.cprice').val();
              $(".batch_no").val(batch_no);
              $(".expdate").val(expdate);
              $(".medprice").val(price);
              $(".medquanav").val(avalablequan);
			}
			});
			  ...
			  $(".medquan").keyup(function(){
				var quan = $(this).val();
				var qa = $(".medquanav").val();
				if(parseInt(qa) >= parseInt(quan)){
			   ...
			   $(".ctsm_grandtotal").val(subtotal);
					...
					alert("maximum quantity available is"+' '+qa);
				}
			 });
...
initSelect2();
			...
			html += '<td><select name="mcat[]" id="mcat'+cat+'" class="form-control item_unit medcates chosen" required=""><option>select</option><?php foreach($category as $ct){?><option value="<?= $ct->id; ?>"><?= $ct->category_name; ?></option><?php } ?></select></td>';
			 ...
			 html += '<td><input type="text" name="expdate[]" value="" id="expdate'+exd+'" class="form-control expdate1 "  required="" ></td>';
			 html += '<td><input type="text" name="medprice[]" value="" id="medprice'+mp+'" class="form-control medprice1"  required="" ></td>';
			 html += '<td  style="display: flex;"><input type="text" name="medquan[]" value="" id="medquan'+mq+'" class="form-control medquan"  required="" style="width:50%;"><input type="text" name="medquanav[]" value="" id="medquanav'+mqa+'" class="form-control medquanav1"  required=""style="width:50%;" ></td>';
	 ...
	 $.ajax({
		...
		$("select#mname"+med).change(function(){
        var medname = $(this).children("option:selected").val();
	 $.ajax({
			url:"<?=base_url('pharmacyController/getproductdetails')?>",
			type:'POST',
			data: {medname:medname},
			success: function(response){
			$(".customproductdata").children().remove()
			$(".customproductdata").html(response);
			var batch_no = $('.cbatch_no').val();
			var avalablequan = $('.cavalablequan').val();
			var expdate = $('.cexpdate').val();
			var price = $('.cprice').val();
              $("#bno"+bno).val(batch_no);
              $("#expdate"+exd).val(expdate);
              $("#medprice"+mp).val(price);
              $("#medquanav"+mqa).val(avalablequan);
			}
			});
