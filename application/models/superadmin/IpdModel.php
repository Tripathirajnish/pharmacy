...
class IpdModel extends CI_model
{
	...
	function getipddetails($pid){
		return $this->db->get_where('ipd', ['patient_id' => $pid])->row();
	}
	function getipddetaildesc($pid){
		//return $this->db->get_where('ipd', ['patient_id' => $pid])->row();
		$this->db->select('*');
		$this->db->from('ipd');
	...
	public function getassignedBedData($checkbed_data){
		$this->db->where($checkbed_data);
